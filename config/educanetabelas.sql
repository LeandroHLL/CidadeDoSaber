...
--
-- Host: 127.0.0.1
-- Tempo de geração: 19/12/2024 às 05:44
...
CREATE TABLE `cadastro` (
  ...
  `status` tinyint(1) NOT NULL DEFAULT 0 COMMENT '1: Aprovado, 0: Pendente, 3: Rejeitado, 4: Ex-Aluno'
...
--
DELIMITER $$
CREATE TRIGGER `atualiza_cadastro_apos_insert` AFTER INSERT ON `senha` FOR EACH ROW BEGIN
    -- Verifica se já existe uma linha correspondente em 'cadastro'
    IF EXISTS (
        SELECT 1 
        FROM cadastro 
        WHERE autenticacao = NEW.autenticacao
    ) THEN
        -- Atualiza a linha existente
        UPDATE cadastro
        SET 
            autenticacao = NEW.autenticacao,  -- Atualiza a coluna autenticacao
            status = CASE 
                        WHEN NEW.situacao = 'Aprovado' THEN 1
                        WHEN NEW.situacao = 'Pendente' THEN 0
                        WHEN NEW.situacao = 'Rejeitado' THEN 3
                        WHEN NEW.situacao = 'Ex-Aluno' THEN 4
                        ELSE status
                     END
        WHERE autenticacao = NEW.autenticacao;
    ELSE
        -- Insere um novo registro em 'cadastro' caso não exista
        INSERT INTO cadastro (autenticacao, status)
        VALUES (
            NEW.autenticacao,
            CASE 
                WHEN NEW.situacao = 'Aprovado' THEN 1
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `atualiza_cadastro_apos_update` AFTER UPDATE ON `senha` FOR EACH ROW BEGIN
    -- Verifica se já existe uma linha correspondente em 'cadastro'
    IF EXISTS (
        SELECT 1 
        FROM cadastro 
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `atualiza_status_cadastro_insert` AFTER INSERT ON `senha` FOR EACH ROW BEGIN
    UPDATE cadastro
    SET status = CASE 
                    WHEN NEW.situacao = 'Aprovado' THEN 1
                    WHEN NEW.situacao = 'Pendente' THEN 0
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `atualiza_status_cadastro_update` AFTER UPDATE ON `senha` FOR EACH ROW BEGIN
    UPDATE cadastro
    SET status = CASE 
                    WHEN NEW.situacao = 'Aprovado' THEN 1
                    WHEN NEW.situacao = 'Pendente' THEN 0
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `update_situacao_aluno_curso` AFTER UPDATE ON `senha` FOR EACH ROW BEGIN
    UPDATE aluno_curso
...
ALTER TABLE `aluno_curso`
  ...
  ADD KEY `aluno_curso_fk_id_cadastro` (`id_cadastro`);
...
ALTER TABLE `aluno_curso`
  ...
  ADD CONSTRAINT `aluno_curso_ibfk_2` FOREIGN KEY (`cod_curso`) REFERENCES `curso` (`cod_curso`) ON DELETE CASCADE ON UPDATE CASCADE;
